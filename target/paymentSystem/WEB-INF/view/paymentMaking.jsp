...
<meta charset="utf-8">
<head>
    <title><fmt:message key="PaymentOperation"/></title>
    ...
    <header id="header" class="header-area">
        ...
        <div class="logoBlockProfile">
            <div style="font-size: 16px; text-align: end;">
                <a class="loginBtn"
                   href="${pageContext.request.contextPath}/view/language/paymentMaking?language=RU">
                    RU
                </a>
                <a class="loginBtn"
                   href="${pageContext.request.contextPath}/view/language/paymentMaking?language=EN">
                    EN
                </a>
            </div>
            <p id="pageLogo" style="font-size: 26px; text-align: center; margin-bottom: 20px"><fmt:message key="PaymentOperation"/></p>
        </div>
    </header>
                                ...
                                <tr class="highlight">
                                    ...
                                    <c:if test="${item.equals(pickedCard)}">
                                        <td><a class="deleteButton"
                                               href="${pageContext.request.contextPath}/view/paymentMaking?command=unpick&Cid=${item.getId()}"><fmt:message key="unpickCard"/></a>
                                        </td>
                                    </c:if>
                                    <c:if test="${!item.equals(pickedCard)}">
                                        <td><a class="updateButton"
                                               href="${pageContext.request.contextPath}/view/paymentMaking?command=pick&Cid=${item.getId()}"><fmt:message key="chooseCard"/></a>
                                        </td>
                                    </c:if>
                                </tr>
                            </c:forEach>
                ...
                <div class="mainmenu" style="margin: unset; text-align: center">
                    <c:if test="${pickedCard != null}">
                        <form class="w3-container" align="center" method="post"
                              action="${pageContext.request.contextPath}/view/paymentMaking?command=pick&Cid=${pickedCard.getId()}">
                            ...
                            <p>
                                <label>
                                    <input class="profileEditorFields loginField" type="text" required
                                           placeholder="<fmt:message key="paymentAmount"/>"
                                           name="paymentAmount">
                                </label>
                            </p>
                            ...
                            <p>
                                <label>
                                    <input class="profileEditorFields loginField" type="text" required placeholder="<fmt:message key="receiverCardId"/>"
                                           name="receiverCardId">
                                </label>
                            </p>
                            ...
                            <c:if test="${requestScope.notEnoughFunds}">
                                <div class="w3-container">
                                    <fmt:message key="notEnoughFunds"/>
                                </div>
                            </c:if>

                            <c:if test="${requestScope.notFoundCard}">
                                <div class="w3-container">
                                    <fmt:message key="notFoundCard"/>
                                </div>
                            </c:if>
                        </form>
                    </c:if>
